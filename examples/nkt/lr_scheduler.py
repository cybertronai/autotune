...
class PolynomialDecayLR(_LRScheduler):
    ...
    def __init__(self, optimizer, rate, max_count, target=None, start_epoch=0, last_epoch=-1):
        self.rate = rate
    ...
    def get_lr(self):
        ...
        decay = max(1-(self.last_epoch-self.start_epoch) / (self.max_count-self.start_epoch), 0)
...
class GradualWarmupLR(_LRScheduler):
    ...
    def __init__(self, optimizer, initial_lr, max_count, last_epoch=-1):
        self.initial_lr = initial_lr
        self.max_count = max_count
        super(GradualWarmupLR, self).__init__(optimizer, last_epoch)

    def get_lr(self):
        if self.last_epoch > self.max_count:
            return [param_group['lr']
                    for param_group in self.optimizer.param_groups]
        else:
            alpha = self.last_epoch / self.max_count
