...
class KFAC_VI(Optimizer):
    """Implements KFAC-VI (variational inference based on NGD) algorithm.
    ...
    Arguments:
        ...
        std_scale (float,optional): scaling rate of posterior's standard deviation 
        num_samples (int,optional): num of samples 
    """
    ...
    def __init__(self, model,
        ...
        if not 0.0 <= std_scale:
            raise ValueError("Invalid std_scale value: {}".format(std_scale))
        if not 0.0 <= num_samples:
            raise ValueError("Invalid num_samples value: {}".format(num_samples))
        ...
        for module in self.train_modules:
            ...
            if isinstance(module,nn.Linear):
                self.fblocks.append(LinearFB(damping=damping,cov_ema_decay=cov_ema_decay,bias=bias,pi_type=pi_type,num_samples=num_samples))
            elif isinstance(module,nn.Conv2d):
                self.fblocks.append(Conv2dFB(kernel_size=module.kernel_size,stride=module.stride,
                    padding=module.padding,dilation=module.dilation,damping=damping,cov_ema_decay=cov_ema_decay,bias=bias,pi_type=pi_type,num_samples=num_samples))
            else:
    ...
    def step(self,inputs,targets,criterion=nn.NLLLoss(), closure=None):
        ...
        if self.fisher_init is False:
            ...
            for fb,kfac_buf in zip(self.fblocks,kfac_bufs):

                if kfac_buf is not None:
                    fb.fisher_init(kfac_buf)
        ...
        for i in range(n):
            ...
            #sampling
            for group in self.param_groups:
                for tp,fb in zip(group['params'],self.fblocks):

                    if fb is None:
                        for p in tp:
                            pass
                    else:
                        fb.sample_param(tp,group['std_scale'])
            ...
            if loss_mean is None:
                loss_mean = loss.data.mul(1/n)
                outputs_mean = outputs.data.mul(1/n)
            else:
                loss_mean.add_(1/n,loss.data)
                outputs_mean.add_(1/n,outputs.data)
        ...
        for group in self.param_groups:
            for tp,fb in zip(group['params'],self.fblocks):
                for p in tp:
                    ...
                    state['step'] += 1
                if fb is not None:
                    fb.update_distribution(group['lr'],group['adaptive_momentum_base'],group['weight_decay_base'])

        return outputs_mean,loss_mean

    def inference(self,inputs,targets,n=None,criterion=nn.NLLLoss(), closure=None):
