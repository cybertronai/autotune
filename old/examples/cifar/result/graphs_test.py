...
def main():
    ...
    def compute_layer_stats(layer):
        ...
        s.sparsity = pos_activations.float()/(pos_activations+neg_activations)
    ...
    u.freeze_layer(model.layers[0])
    u.freeze_layer(model.layers[2])
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for i in range(len(model.layers)):
            ...
            layer_stats = compute_layer_stats(model.layers[i])
            layer_name = f"{i:02d}-{'layername'}"   # todo(y): layer name
            for (stat_name, value) in layer_stats.items():
                model_stats[f'{layer_name}/{stat_name}'] = value

        log(model_stats, step=num_examples_processed)
