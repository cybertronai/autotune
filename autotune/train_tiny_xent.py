...
def main():
    ...
    dataset = u.TinyMNIST(data_width=args.data_width, targets_width=args.targets_width, dataset_size=dataset_size, loss_type=loss_type)

    train_loader = torch.utils.data.DataLoader(dataset, batch_size=args.train_batch_size, shuffle=False, drop_last=True)
    train_iter = u.infinite_iter(train_loader)

    stats_iter = stats_loader = None
    ...
    test_dataset = u.TinyMNIST(data_width=args.data_width, targets_width=args.targets_width, train=False, dataset_size=dataset_size, loss_type=loss_type)
            ...
            for (i, layer) in enumerate(model.layers):
                ...
                with u.timeit(f'rho-{i}'):
                    ...
                    if u.has_nan(p_sigma):
                        ...
                        s.psigma_erank = u.erank(p_sigma)
                ...
                p_sigma_svals = u.svd_pos_svals(p_sigma)
                u.log_spectrum(f'hess-{i}', H_evals)
                u.log_spectrum(f'sigma-{i}', sigma_evals)
                u.log_spectrum(f'psigma-{i}', p_sigma_svals)
